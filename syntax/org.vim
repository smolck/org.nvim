" From https://github.com/jceb/vim-orgmode/blob/master/syntax/org.vim, with
" slight changes. Credit goes to the maintainers of vim-orgmode.
...
" Don't change the background of folds from everything else
" so it looks like normal text (as in Emacs' Org Mode)
...
if g:org_heading_shade_leading_stars == 1
    ...
    " Requires `:set conceallevel=1`
	syntax match org_shade_stars /\v\*(\*)@=/ contained conceal
...
while s:i <= g:org_heading_highlight_levels
    " TODO(smolck): Make the `*` character appear like a bullet point or similar
    " exec 'syntax match stuff' . s:i . ' /\*\%(^\*\{' . (s:i) . '}\)\@<=/' . ' conceal cchar=•'
    " exec 'hi def link stuff' . s:i .
...
" vi: ft=vim:tw=80:sw=4:ts=4:fdm=markere
